...
import com.cellrox.infra.pages.CellRoxPoliciesPage;
...
public class MDMOperation extends TestCase {
	...
	private boolean businessContacts;
	private boolean businessCalendar;
	private DataSharingProperty[] dataSharingProperties;

	/**
	 * This is step 1 in the activation. The function : 1. Push the file of
	 * unenroll.zip to the /sdcard 2. Execute : echo 'boot-recovery ' >
	 * /cache/recovery/command. 3. Execute : echo '--update_package=" +
	 * mobileFileLocation + "'>> /cache/recovery/command. 4. Reboot recovery. 5.
	 * Wait for the device to return up. 6. Configure device with priv 7.
	 * Connect to servers 8. Click on corp persona and check that it isn't
	 * activated.
	 * 
	 * */
	@Test
	@TestProperties(name = "Unactivate Device With unenroll.zip", paramsInclude = { "currentDevice" })
	// @TestProperties(name = "Unactivate Device With unenroll.zip",
	// paramsInclude = {"currentDevice,localLocation"})
	public void unactivateDevice() throws Exception {
		// step 1
		// final String mobileFileLocation = "/sdcard/unenroll.zip";

		// push
		// devicesMannager.getDevice(currentDevice).pushFileToDevice(localLocation.getAbsolutePath(),
		// mobileFileLocation);
		// the two lines
		...
		// + mobileFileLocation + "'>> /cache/recovery/command");
		// reboot recovery
	...
	@Test
	@TestProperties(name = "Add New Policy ${policyName}", paramsInclude = { "policyName" })
	public void addNewPolicy() throws Exception {
		CellRoxPoliciesPage policiesPage = new CellRoxPoliciesPage(driver, siteUrl);
		policiesPage.addPolicy(policyName);
		report.report("New Policy " + policyName + " was Created Successfully");
	}

	@Test
	@TestProperties(name = "Delete Policy ${policyName}", paramsInclude = { "policyName" })
	public void deletePolicy() throws Exception {
		CellRoxPoliciesPage policiesPage = new CellRoxPoliciesPage(driver, siteUrl);
		policiesPage.deletePolicy(policyName);
		report.report("\"" + policyName + " was Deleted Successfully");
	}

	@Test
	@TestProperties(name = "Edit Data Sharing for Policy ${policyName}", paramsInclude = { "policyName, businessContacts, businessCalendar, dataSharingProperties" })
	public void editDataSharingPolicy() throws Exception {
		CellRoxPoliciesPage policiesPage = new CellRoxPoliciesPage(driver, siteUrl);
		policiesPage.editDataSharingPolicy(policyName, businessContacts, businessCalendar, dataSharingProperties);
		report.report("\"" + policyName + " was Deleted Successfully");
	}

	/**
	 * This is step 2 in the activation. The function will remove the primary
	 * device from the MDM and verify it
	 * */
	@Test
	...
	public void removeDevicefromMdmDeviceList() throws Exception {
		...
		// to check the imei of the device and find the correct device, to click
		// the check box
	...
	/**
	 * This is step 3 in the activation. The function : 1. Click on the owner
	 * and enroll the device 2. Get the activation code from the MDM 3. Activate
	 * the phone. 4. Restart the phone. 5. Configure the device for uiautomator.
	...
	public void enrollOwner() throws Exception {
		...
		// to get the https://mdm-qa.cellrox.com/automation/devices.json and to
		// get the activation_code
		// to click add information , to enter to the device, click on the next
		// and than on reboot
	...
	public void validateActivatedPersona() throws Exception {
		...
		// to go to the mdm devices and to click on the device and find the
		// results, max time is 5 min
	}

	/**
	 * This is step 5 in the activation. This function : 1. change the policy to
	 * the wanted policy 2. validate from the MDM that the policy changed
	 * */
	@Test
	...
	public void pushPolicy() throws Exception {
		...
		// select the wanted device in the device page, click on change policy,
		// choose the wanted policy
		// click apply
		// to click on the device , click on history , check done is in the
		// coosen policy
...
//	}
	...
	public boolean isBusinessContacts() {
		return businessContacts;
	}

	public void setBusinessContacts(boolean businessContacts) {
	...
	public void setBusinessCalendar(boolean businessCalendar) {
		this.businessCalendar = businessCalendar;
	}

	public DataSharingProperty[] getDataSharingProperties() {
		return dataSharingProperties;
	}

	@UseProvider(provider=jsystem.extensions.paramproviders.ObjectArrayParameterProvider.class)
	public void setDataSharingProperties(DataSharingProperty[] dataSharingProperties) {
		this.dataSharingProperties = dataSharingProperties;
	}
